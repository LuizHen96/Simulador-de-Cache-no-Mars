...
import mars.cache.swing.dinamiccache.Block;
import mars.cache.swing.dinamiccache.Set;
import mars.cache.swing.dinamiccache.Word;
...
public class Cache extends JFrame{
	...
	public Cache(int nSets, int nBlocks, int nWords, int x, int y, StatisticCache statisticCache)
	{
		this.nSets = nSets;
		...
		sets = new Set[nSets];
		values = new String[this.nBlocks][nWords];
		for(int i=0; i<this.nBlocks; i++)
			for(int j=0; j<this.nWords; j++)
				values[i][j] = new String(statisticCache.getnBlocos() + "");
		...
		{
			sets[i] = new Set(this.nBlocks, nWords, (HEIGHT_SET*i)+WITDH_INDEX+ INDEX, HEIGHT_NAMES, values,statisticCache.getSet()[i]);
		}
